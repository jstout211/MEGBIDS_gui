...
def write_mri_bids(meg_fname=None, mri_deface_fname=None, session=None,
                   bids_subject=None, bids_root=None, transform_fname=None,
                   ):
    ...
    t1w_bids_path = \
        BIDSPath(subject=bids_subject, session=session, root=bids_root, suffix='T1w')
    
    subjects_dir = os.environ['SUBJECTS_DIR']
    tmp_subjects_dir = '/tmp/defaced_subjs_dir'
    if not os.path.exists(tmp_subjects_dir): os.mkdir(tmp_subjects_dir)
    os.environ['SUBJECTS_DIR']=tmp_subjects_dir
    if not os.path.exists(os.path.join(tmp_subjects_dir, 'tmp_defaced')):
        subprocess.run(f'recon-all -i {mri_deface_fname} -s tmp_defaced -autorecon1'.split(' '))
...
def test_write_mri_bids():
    ...
    transform_fname = os.path.expanduser('~/src/GUI_testdata/sample_audvis_trunc-trans.fif')
    mri_deface_fname = '/tmp/deface/defaced_sub-01_T1w.nii.gz'
    session='01'
    run='01'
    bids_root='/tmp/bids_root'
    bids_subject='S01'
    write_mri_bids(meg_fname=meg_fname, mri_deface_fname='/tmp/deface/defaced_sub-01_T1w.nii.gz', session='01',
                   bids_subject='S01', bids_root='/tmp/bids_root', transform_fname=transform_fname,
                   )
